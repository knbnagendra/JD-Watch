...
def test_targets_are_hardcoded_never_from_config():
    ...
    assert killswitch_dryfire.TARGETS == ("tradier_sandbox", "alpaca_sandbox")
    assert "tradier_live" not in killswitch_dryfire.TARGETS
...
def test_successful_dry_fire_halts_then_rearms_both_targets_no_alert(ctx):
    ...
    assert set(ctx.relay.halt_calls) == {("tradier_sandbox", False), ("alpaca_sandbox", False)}
    assert set(ctx.relay.rearm_calls) == {"tradier_sandbox", "alpaca_sandbox"}
    # Final state confirmed cleared for both.
    assert ctx.relay.halted == {"tradier_sandbox": False, "alpaca_sandbox": False}
...
def test_halt_never_confirmed_alerts_critical_but_still_rearms(ctx):
    ...
    ctx.relay.get_status = lambda: {"accounts": {"tradier_sandbox": {"manual_halt": False},
...
def test_success_resolves_any_prior_alert_state(ctx):
    ctx.alerter.alert(f"{killswitch_dryfire.NAME}:tradier_sandbox", Severity.CRITICAL, "prior failure", now=0.0)
    killswitch_dryfire.run(ctx)
    assert f"{killswitch_dryfire.NAME}:tradier_sandbox" in ctx.alerter.resolved
