...
- **Agni (India's 0DTE product) currently has no regime or safety gate at all.** This is explicit and documented in the code as a must-fix-before-scaling item, not an oversight nobody noticed — wiring it through the existing (GEX-based) regime engine unmodified would simply never let it trade at all, since India has no GEX data. A dedicated gate for it (liquidity floor, volatility gate, event blackout) has not yet been built.
...
Honestly: it's a genuine, documented gap, not an oversight. See [§11](#11-known-limitations--honest-gaps).
